...
SILERO_AVAILABLE = True  # torch가 있으면 자동 다운로드
...
class LongVideoAudioPreprocessor:
    ...
    def __init__(self, config_path: str = "video_analyzer/configs/inference_config.yaml"):
        ...
        self.logger.info(f"   Silero VAD 신뢰도: {self.vad_confidence_threshold}")
    
    def _init_vad(self):
        """Silero VAD 초기화 (torch.hub 기반)"""
        try:
            # torch.hub에서 Silero VAD 모델 로드
            ...
            self.vad_confidence_threshold = self.config['audio'].get('vad_confidence_threshold', 0.5)
            ...
            self.logger.info(f"✅ Silero VAD 초기화 완료 (torch.hub)")
            self.logger.info(f"   디바이스: {device}")
            self.logger.info(f"   신뢰도 임계값: {self.vad_confidence_threshold}")
            
        except Exception as e:
            self.logger.error(f"❌ Silero VAD 초기화 실패: {e}")
            self.logger.info("💡 인터넷 연결을 확인하고 torch가 설치되어 있는지 확인하세요")
            raise
    ...
    def _extract_vad_sequence(self, y: np.ndarray, sr: int) -> np.ndarray:
        """
        Silero VAD를 사용한 VAD 시퀀스 추출 (torch.hub 기반)
        ...
        """
        if sr != 16000:
            raise ValueError(f"Silero VAD는 16kHz만 지원합니다. 현재: {sr}Hz")
        ...
        try:
            ...
            # Silero VAD로 발화 구간 탐지 (torch.hub utils 사용)
            speech_timestamps = self.get_speech_timestamps(audio_tensor, 
                                                          self.vad_model,
                                                          sampling_rate=sr,
                                                          threshold=self.vad_confidence_threshold,
                                                          speech_pad_ms=50,  # 발화 전후 50ms 패딩
                                                          return_seconds=True)
            
            # 전체 오디오 길이에 대한 VAD 레이블 생성
            total_samples = len(y)
            samples_per_interval = int(sr * self.analysis_interval)  # 0.05초당 샘플 수
            num_intervals = total_samples // samples_per_interval
            
            vad_labels = np.zeros(num_intervals, dtype=int)
            
            # 발화 구간을 0.05초 단위로 매핑
            for speech_dict in speech_timestamps:
                start_second = speech_dict['start']
                end_second = speech_dict['end']
                
                start_interval = int(start_second / self.analysis_interval)
                end_interval = int(end_second / self.analysis_interval)
                
                # 범위 체크
                start_interval = max(0, start_interval)
                end_interval = min(end_interval, num_intervals - 1)
                
                # 해당 구간을 발화로 마킹
                if start_interval <= end_interval:
                    vad_labels[start_interval:end_interval + 1] = 1
            
            return vad_labels
            
        except Exception as e:
            self.logger.error(f"❌ Silero VAD 처리 실패: {e}")
    ...
    def _save_to_hdf5(self, result: Dict) -> str:
        ...
        try:
            ...
            with h5py.File(hdf5_path, 'w') as f:
                ...
                # Silero VAD 설정 메타데이터 추가
                f.attrs['vad_model'] = 'silero_torch_hub'
                f.attrs['vad_confidence_threshold'] = self.vad_confidence_threshold
    ...
    def _save_audio_statistics(self, result: Dict):
        ...
        try:
            ...
            with open(stats_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"⚙️ Silero VAD 설정\n")
                f.write(f"├─ 모델: Silero VAD (torch.hub)\n")
                f.write(f"└─ 신뢰도 임계값: {self.vad_confidence_threshold}\n")
...
def main():
    ...
    # Silero VAD torch.hub 기반 확인
    if not torch.cuda.is_available():
        print("⚠️ CUDA가 없어도 CPU에서 동작합니다")
    
    print("✅ torch.hub에서 Silero VAD를 자동 다운로드합니다")
